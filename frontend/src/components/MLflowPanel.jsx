    ...
    return (
        ...
        >
            <div className="flex items-start justify-between mb-5 flex-wrap gap-3">
                <div>
                    ...
                    <div className="font-display text-xl font-bold text-c1b-primary tracking-tight flex items-center gap-2">
                        <Activity className="w-5 h-5 text-c1b-accent" />
                        Model Drift Monitor
                    </div>
                </div>
                <div className="flex items-center gap-2">
                    ...
                    {isAdmin && (
                        <Button
                            ...
                            className="bg-c1b-accent hover:bg-c1b-accent-soft text-white hover:scale-[1.02] transition-all"
                        >
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
                <div className="p-3 rounded-xl bg-c1b-surface" data-testid="mlflow-baseline-auc">
                    <div className="text-[10px] uppercase tracking-wider text-c1b-muted">Baseline AUC</div>
                    <div className="font-mono text-lg font-bold text-c1b-primary mt-1">
                        {drift?.baseline_auc?.toFixed(4) ?? "—"}
                    </div>
                </div>
                <div className="p-3 rounded-xl bg-c1b-surface" data-testid="mlflow-current-auc">
                    <div className="text-[10px] uppercase tracking-wider text-c1b-muted">Current AUC</div>
                    <div className="font-mono text-lg font-bold text-c1b-primary mt-1">
                        {drift?.current_auc?.toFixed(4) ?? "—"}
                    </div>
                </div>
                <div className="p-3 rounded-xl bg-c1b-surface" data-testid="mlflow-auc-drop">
                    <div className="text-[10px] uppercase tracking-wider text-c1b-muted flex items-center gap-1">
            ...
            <p className="text-xs text-c1b-muted leading-relaxed mb-4">{drift?.message}</p>

            <div>
                <div className="text-[11px] uppercase tracking-[0.18em] text-c1b-muted mb-2 flex items-center gap-1">
                    <History className="w-3.5 h-3.5" /> Recent Runs
                </div>
                <div className="border border-c1b-border rounded-xl bg-white overflow-hidden">
                    <div className="grid grid-cols-12 px-4 py-2 text-[10px] uppercase tracking-wider text-c1b-muted bg-c1b-surface border-b border-c1b-border">
                    ...
                    {runs.map((r, i) => {
                        ...
                        return (
                            <motion.div
                                ...
                                className={`grid grid-cols-12 px-4 py-2.5 items-center text-xs border-b border-c1b-border last:border-b-0 ${isBase ? "bg-c1b-accent/5" : ""}`}
                                data-testid={`mlflow-run-${i}`}
                            >
                                <div className="col-span-4 font-mono text-c1b-primary truncate flex items-center gap-1.5">
                                    {isBase && <Pin className="w-3 h-3 text-c1b-accent" />}
                                    {r.run_id.slice(0, 16)}…
                                </div>
                                <div className="col-span-2 text-c1b-muted">{r.trigger}</div>
                                <div className="col-span-2 font-mono font-semibold text-c1b-primary">{r.roc_auc.toFixed(4)}</div>
                                <div className="col-span-3 text-c1b-muted text-[11px]">
                                ...
                                <div className="col-span-1 text-right">
                                    {isAdmin && !isBase && (
                                        <button
                                            ...
                                            className="text-c1b-muted hover:text-c1b-accent transition"
                    ...
                    {runs.length === 0 && (
                        <div className="p-4 text-center text-xs text-c1b-muted">No runs yet — trigger a retrain.</div>
                    )}
                </div>
            </div>
        </motion.div>
